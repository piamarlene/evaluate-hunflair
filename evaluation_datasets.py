...
class NLM_CHEM:
    ...
    def get_annotated_sentences(self, ner_only: bool = False):
        self.annotated_sentences = bigbio_dataset_to_annotated_flair_sentences(
            self.dataset, self.tokenizer, add_labels=True, ner_only=ner_only
        )
        return self.annotated_sentences
...
class BIGBIO_PDR:
    def __init__(self, use_tokenizer, split="validation"):
        conhelps = BigBioConfigHelpers()
        dataset = conhelps.for_config_name("pdr_bigbio_kb").load_dataset()
        self.dataset = dataset[split]
        self.tokenizer = use_tokenizer
        self.sentences = bigbio_dataset_to_annotated_flair_sentences(
            self.dataset, tokenizer=use_tokenizer, add_labels=False
...
class NEN_NCBI_DISEASE:
    ...
    def get_annotated_sentences(
        self, ner_only: bool = False, use_corpus_label_name: bool = False
    ):
        if use_corpus_label_name:
            self.annotated_sentences = bigbio_dataset_to_annotated_flair_sentences(
                self.dataset, self.tokenizer, add_labels=True, ner_only=ner_only
            )
        else:
